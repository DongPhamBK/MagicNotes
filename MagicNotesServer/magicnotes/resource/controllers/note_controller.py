...
class NoteController(Resource):
    ...
    def get(self, user_ref=user_ref, note_ref=note_ref):
        try:
            ...
            notes_raw = note_ref.where('user', '==', user).stream()
            # Chuyển đổi dữ liệu
            ...
            return Response(400, "error", [], str(e)).json
    ...
    def post(self, user_ref=user_ref, note_ref=note_ref):
        try:
            ...
            # print(user)
            # truy vấn với reference
            new_note = Note(uuid.uuid4().hex, note_title, note_description, note_created_time, NoteState.get_state(note_state), user)
            # print(note.__str__())
    ...
    def put(self, user_ref=user_ref, note_ref=note_ref):
        try:
            ...
            # Quá trình cập nhật
            note_updated = Note(uuid.uuid4().hex, note_title, note_description, note_created_time, NoteState.get_state(note_state), user)
            note_ref.document(note_id).update(note_updated.json)
            return Response(200, "success", "null", f"Update note successful!!").json
        except Exception as e:
    ...
    def delete(self, note_ref=note_ref):
        try:
            ...
            return Response(200, "success", "null", f"Delete note successful!!").json
        except Exception as e:
            return Response(400, "error", "null", str(e)).json
